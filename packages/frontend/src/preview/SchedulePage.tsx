...
function ShortScheduleList({ actions, schedules }: { actions: PreviewActions; schedules: Schedule[] }) {
  ...
  return <section className="short-schedule-list"><h2>短时课程</h2>{short.map((item) => <Button variant="outline" key={item.id} className={`short-schedule-card status-${statusClass(item.status)}`} onClick={() => openScheduleDetails(actions, item)} aria-label={`查看 ${item.start} 至 ${item.end} ${scheduleObjectLabel(actions.data, item)} ${item.location || '待补充地点'} ${statusLabel(item.status)} 排期详情`}><dl><div><dt>时间</dt><dd>{formatDate(item.day)} {item.start}–{item.end}</dd></div><div><dt>地点</dt><dd>{item.location || '待补充'}</dd></div><div><dt>对象</dt><dd>{scheduleObjectLabel(actions.data, item)}</dd></div></dl><Badge variant="secondary" className="schedule-status">{statusLabel(item.status)}</Badge></Button>)}</section>;
}
...
function ScheduleList({ actions, schedules }: { actions: PreviewActions; schedules: Schedule[] }) {
  ...
  return <div className="schedule-list">{schedules.map((item) => <Button variant="outline" className={`schedule-list-card status-${statusClass(item.status)}`} key={item.id} onClick={() => openScheduleDetails(actions, item)} aria-label={`查看 ${formatDate(item.day)} ${item.start} 至 ${item.end} ${scheduleObjectLabel(actions.data, item)} ${item.location || '待补充地点'} ${statusLabel(item.status)} 排期详情`}><dl><div><dt>时间</dt><dd>{formatDate(item.day)}<br /><time>{item.start}–{item.end}</time></dd></div><div><dt>地点</dt><dd>{item.location || '待补充'}</dd></div><div><dt>对象</dt><dd>{scheduleObjectLabel(actions.data, item)}</dd></div></dl><Badge variant="secondary" className="schedule-status">{statusLabel(item.status)}</Badge><span className="schedule-list-action" aria-hidden="true">查看详情 ›</span></Button>)}</div>;
}
...
function RuleList({ actions }: { actions: PreviewActions }) {
  ...
  return <section className="recurrence-rules"><h2>重复规则</h2>{actions.data.recurrenceRules.map((rule) => {
    const ended = Boolean(rule.endDate && rule.endDate < today);
    return <Card className="white-card" key={rule.id}><Button variant="ghost" className="rule-summary" onClick={() => openDetails(rule)}><b>{ended ? '已结束' : rule.enabled ? '已启用' : '已暂停'} · 每周 {ruleDays(rule)}</b><span>{rule.start}–{rule.end} · {ruleObject(actions, rule)} · {rule.location || '待补充'}</span><small>{formatDate(rule.startDate)} 起{rule.endDate ? `，至 ${formatDate(rule.endDate)}` : '，无结束日期'}</small></Button><div className="rule-actions"><Button variant="outline" size="sm" onClick={() => openDetails(rule)}>详情</Button><Button variant="outline" size="sm" disabled={ended} onClick={() => toggle(rule)}>{ended ? '规则已结束' : rule.enabled ? '暂停规则' : '启用规则'}</Button></div></Card>;
  })}</section>;
}
