...
export function SettingsPage({ actions, modelSettings }: { actions: PreviewActions; modelSettings?: ReactNode }) {
  ...
  const subtitles: Record<SettingsRoute, string> = { studio: '管理工作室名称。', models: modelSettings ? '查看平台 DeepSeek 服务与当前模型配置。' : '选择助手的响应偏好。', wechat: '', privacy: '了解数据使用与服务边界。' };
  return <section className="page preview-page settings-configuration"><header className="page-header"><div><h1>{route === 'studio' ? '设置' : route === 'models' ? modelSettings ? '模型与 API' : '模型选择' : route === 'wechat' ? '微信连接' : '数据与隐私'}</h1><p>{subtitles[route]}</p></div><Button asChild variant="outline"><a href="#/today">返回工作台</a></Button></header><div className="settings-layout"><SettingsNav route={route} connectedModels={Boolean(modelSettings)} /><div>{route === 'studio' && <div className="settings-stack"><StudioSettings actions={actions} />{modelSettings && <Card className="white-card settings-form"><CardContent><h2>模型与 API</h2><p>查看平台 DeepSeek 服务状态；教师无需管理 API Key。</p><Button asChild variant="outline"><a href="#/settings/models">管理模型与 API</a></Button></CardContent></Card>}</div>}{route === 'models' && (modelSettings || <ModelSettings actions={actions} />)}{route === 'wechat' && <WechatSettings actions={actions} />}{route === 'privacy' && <PrivacySettings />}</div></div></section>;
}
...
function UnavailableCheck({ checked, onCheck, subject, status }: { checked: boolean; onCheck: () => void; subject: string; status: string }) {
  return <Card className="white-card unavailable-card"><CardContent><h2>{subject}</h2><p className="check-result" role="status">{status}</p><Button type="button" variant="outline" onClick={onCheck}>{checked ? '重新检查' : '检查状态'}</Button></CardContent></Card>;
}
