...
export function FinancePage({ actions }: { actions: PreviewActions }) {
  ...
  return <section className="page preview-page finance-page">
    <header className="page-header"><h1>缴费课时</h1><Button onClick={register}>登记缴费</Button></header>
    ...
    <div className="finance-stats" aria-label={studentId ? `${studentName(actions.data, studentId)}的课时概览` : '全部学生课时概览'}>
      <Card><CardContent><span>{studentId ? '当前学生' : '学生总数'}</span><strong>{studentId ? studentName(actions.data, studentId) : students.length}</strong></CardContent></Card>
      <Card><CardContent><span>{studentId ? '续费状态' : '待续费提醒'}</span><strong>{studentId ? students[0]?.balance <= 3 ? '待续费' : '充足' : students.filter((s) => s.balance <= 3).length}</strong></CardContent></Card>
      <Card><CardContent><span>缴费记录</span><strong>{records.length}</strong></CardContent></Card>
      <Card><CardContent><span>{studentId ? '剩余课时' : '剩余总课时'}</span><strong>{students.reduce((sum, student) => sum + student.balance, 0)}</strong></CardContent></Card>
    </div>
    {!records.length ? <Card className="white-card finance-empty"><CardContent><h2>暂无缴费记录</h2><p>{studentId ? `${studentName(actions.data, studentId)}还没有缴费记录。` : '登记第一笔缴费后，可在这里查看课时变化。'}</p><Button variant="outline" onClick={register}>登记第一笔缴费</Button></CardContent></Card> : <Card className="white-card schedule-table finance-table"><CardContent><div className="schedule-table-scroll"><table><thead><tr><th>学生</th><th>金额</th><th>增加课时</th><th>登记日期</th></tr></thead><tbody>{records.map((payment) => <tr key={payment.id}><td data-label="学生">{studentName(actions.data, payment.studentId)}</td><td data-label="金额" className="money">¥{payment.amount}</td><td data-label="增加课时">+{payment.lessons}</td><td data-label="登记日期"><time dateTime={payment.date}>{formatDate(payment.date)}</time></td></tr>)}</tbody></table></div></CardContent></Card>}
  </section>;
}
