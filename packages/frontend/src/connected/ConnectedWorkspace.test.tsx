...
describe('connected workspace server-backed writes', () => {
  ...
  it('mounts the authoritative student timeline and refreshes it with the workspace', async () => {
    location.hash = '#/students/s1';
    mock.records.mockResolvedValue({ items: [], total: 0 });
    mock.timeline.mockResolvedValue({ items: [{
      type: 'assessment', id: 'assessment-server', occurredAt: '2026-09-20T01:00:00.000Z',
    ...
    fireEvent.keyDown(screen.getByRole('button', { name: '账号菜单' }), { key: 'Enter' });
    fireEvent.click(await screen.findByRole('menuitem', { name: '刷新资料' }));
    await waitFor(() => expect(mock.timeline.mock.calls.length).toBeGreaterThan(before));
  });
