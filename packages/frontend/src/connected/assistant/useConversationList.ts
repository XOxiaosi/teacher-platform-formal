...
import { createConversation, getConversation, listConversations, type ConversationResponse, type ConversationStatus, type ConversationSummaryDto } from '../../api/conversations';
...
export function useConversationList(teacherId: string, status: ConversationStatus, transport?: AssistantTransport) {
  ...
  async function load(append = false) {
    if (lock.current) {
      // Opening history while the first page is still loading must still fetch
      // a current summary once that request settles. Pagination is already
      // disabled while busy, so only queue a replacement first page.
      if (!append) queuedRefresh.current = true;
      return;
    }
    lock.current = true;
  ...
  useEffect(() => { createdConversationId.current = null; }, [teacherId]);
  async function create(): Promise<string | null> {
    ...
    try {
      ...
      if (transport?.createConversation) {
        const id = createdConversationId.current ?? await transport.createConversation({ teacherId });
        createdConversationId.current = id;
        response = await (conversationApi?.detail ?? getConversation)(teacherId, id);
        createdConversationId.current = null;
      } else response = await createConversation(teacherId);
