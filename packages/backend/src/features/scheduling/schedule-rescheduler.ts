...
function toScheduleData(record: Schedule): ScheduleData {
  return {
    ...
    participantIds: record.studentId ? [record.studentId] : [],
    type: record.type,
    title: record.title,
    // This legacy edit path does not decrypt protected course work fields.
    // Formal web reads go through ScheduleService/Agenda instead.
    location: null,
    classFormat: record.classFormat === 'one_to_one' || record.classFormat === 'small_group' ? record.classFormat : null,
...
function replacementData(
  ...
  return {
    ...
    type: before.type,
    title: before.title,
    locationCiphertext: before.locationCiphertext,
    classFormat: before.classFormat,
    operationalNoteCiphertext: before.operationalNoteCiphertext,
