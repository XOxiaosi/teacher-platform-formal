...
describe('认证边界（HTTP 层）', () => {
  it('密码过短注册 → 400 field=password 且不种 cookie', async () => {
    const res = await request(app)
      .post('/api/v1/auth/register')
      .send({ email: uniqueEmail(), password: 'short', displayName: 'A' });
  ...
  it('邮箱格式错误注册 → 400 field=email', async () => {
    const res = await request(app)
      .post('/api/v1/auth/register')
      .send({ email: 'not-an-email', password: 'password123', displayName: 'A' });
  ...
  it('会话过期后 /me → 401（过期判定在服务端，TrustedClock 兜底）', async () => {
    ...
    const reg = await agent
      .post('/api/v1/auth/register')
      .send({ email: uniqueEmail(), password: 'password123', displayName: '过期老师' });
    expect(reg.status).toBe(201);
    trackTeacher(reg.body.data.teacher);
  ...
  it('登出后旧 sessionToken 立即失效（DB 删除，重放旧 cookie → 401）', async () => {
    ...
    const reg = await agent
      .post('/api/v1/auth/register')
      .send({ email: uniqueEmail(), password: 'password123', displayName: '登出老师' });
    expect(reg.status).toBe(201);
    trackTeacher(reg.body.data.teacher);
...
describe('生产模式红线（NODE_ENV=production）', () => {
  ...
  it('A11 生产模式：有效 session cookie → /me 200', async () => {
    ...
    const reg = await request(app)
      .post('/api/v1/auth/register')
      .send({ email, password: 'password123', displayName: '生产老师' });
    expect(reg.status).toBe(201);
    trackTeacher(reg.body.data.teacher);
