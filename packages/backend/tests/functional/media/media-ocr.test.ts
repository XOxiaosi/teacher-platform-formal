...
async function waitForOcrStatus(
  ...
  expected: string,
  timeoutMs = 3000,
): Promise<{ status: string; text: string | null; blocks: unknown; jobStatus: string | null }> {
  const deadline = Date.now() + timeoutMs;
  for (;;) {
    const poll = await service.getOcr(TEACHER, assetId);
    if (!poll.ok) throw new Error(`轮询失败: ${poll.error.message}`);
    const { ocrStatus, ocrText, ocrLayoutBlocks, job } = poll.value;
    if (ocrStatus === expected) {
      return { status: ocrStatus, text: ocrText, blocks: ocrLayoutBlocks, jobStatus: job?.status ?? null };
    }
...
describe('P11：OCR 接线（shared/platform-services adapter 替换占位）', () => {
  ...
  async function poll(
    ...
    for (;;) {
      ...
      const { ocrStatus, ocrText, ocrLayoutBlocks, job } = pollResult.value;
      if (ocrStatus === expected) {
        return { status: ocrStatus, text: ocrText, blocks: ocrLayoutBlocks, jobStatus: job?.status ?? null };
      }
      if (Date.now() > deadline) {
