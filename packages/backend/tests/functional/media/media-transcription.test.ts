...
async function waitForTranscriptionStatus(
  ...
  expected: string,
  timeoutMs = 3000,
): Promise<{ status: string; text: string | null; jobStatus: string | null }> {
  const deadline = Date.now() + timeoutMs;
  for (;;) {
    const poll = await service.getTranscription(TEACHER, assetId);
    if (!poll.ok) throw new Error(`轮询失败: ${poll.error.message}`);
    const { transcriptionStatus, transcriptionText, job } = poll.value;
    if (transcriptionStatus === expected) {
      return { status: transcriptionStatus, text: transcriptionText, jobStatus: job?.status ?? null };
    }
...
describe('P10：ASR 转写接线（shared/platform-services adapter 替换占位）', () => {
  ...
  async function poll(
    ...
    for (;;) {
      ...
      const { transcriptionStatus, transcriptionText, job } = pollResult.value;
      if (transcriptionStatus === expected) {
        return { status: transcriptionStatus, text: transcriptionText, jobStatus: job?.status ?? null };
      }
      if (Date.now() > deadline) {
  ...
  it('retryable（rate_limited）→ 指数退避重试后成功（2 次失败 + 第 3 次成功）', async () => {
    ...
    const flakyAsr: AsrAdapter = {
      ...
      async transcribe() {
        ...
        return { text: '重试后成功转写', confidence: 0.8 };
      },
    };
