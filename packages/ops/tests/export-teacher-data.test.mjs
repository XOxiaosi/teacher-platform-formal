...
test('BUSINESS_TABLES：46 个业务表固定清单（教师库 40 + 共享库 6）', () => {
  assert.equal(BUSINESS_TABLES.length, 46);
  ...
  assert.ok(BUSINESS_TABLES.includes('LessonStatusCorrectionConfirmation'));
...
test('end-to-end：--teacher-id 导出 → account.json 无 passwordHash + manifest 行数正确 + 媒体文件本体', async () => {
  ...
  // manifest：46 表 + 行数正确 + sha256 存在 + 来源库标注
  const manifest = JSON.parse(await readFile(join(outDir, 'manifest.json'), 'utf8'));
  assert.equal(manifest.tables.length, 46);
...
test('P14 t5 e2e：--zip 输出单包（manifest+account+tables+media）→ 解包结构/内容/CRC 与目录一致', { skip: process.platform !== 'win32' }, async () => {
  ...
  // 结构：manifest.json / account.json / tables/*.jsonl（46）/ media/*
  ...
  assert.equal(unzipTables.length, 46);
