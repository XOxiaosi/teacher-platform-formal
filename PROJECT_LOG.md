...
| 当前任务 | A02-WORKER / A03-REC2 / A04-SRC：教学任务 worker、助手恢复边界与来源失效 |
| 当前任务状态 | 进行中（本地 worker/心跳、助手恢复刷新/登出清理、Capture 来源失效已验证；真实 DSH、跨设备、多候选迁移和用户验收另行验收） |
...
| 验证概况 | GOV-004 第二轮完整门禁：治理 19 项；后端 303 文件/2688 项、前端 39 文件/233 项、管理端 13 文件/84 项、运维 124 项通过；运维 2 项 Windows 专属测试跳过。类型检查、lint、构建、隔离 PostgreSQL 17 全部通过；A04 来源失效用例包含在后端全量通过中 |
| 下一步 | 先定义并验证真实 DeepSeek/DSH adapter，再做 A03 跨设备/重登验收；A04 多候选兼容迁移与来源准入继续保持未开始，不以本地单候选修复替代 |
| 长任务目标及结束条件 | 业务长任务沿用 V009 当前执行计划及已有授权，以对应产品验收和实际证据收口；单个工作包/commit 不代表整体完成。本次 GOV-004 仅优化连续执行规则、日志和治理检查，验证并提交后结束，不启动业务实现 |
| 当前可执行任务 | GOV-004 治理改动与完整验证已完成，提交关联本任务编号；本次无剩余业务实现任务。业务恢复时重新核对 A02 adapter 本地契约验证、A03 合成重登/恢复检查的依赖与剩余缺口，再选择可执行任务；本次不将候选任务标为已启动 |
...
- 本轮授权：用户要求自主继续推进，并明确要求拆分多个子 Agent 并行、由主 Agent 检查；本轮执行 V009 既定 A02-INT/A03-REC，使用本地合成运行时、隔离 PostgreSQL 和正式入口完成可回滚工程改动，测试通过后分别创建 commit。真实 DSH、正式写操作、跨设备、UI 效果和渠道仍分别验收，不能混报完成。保留已有无关源码修改。
