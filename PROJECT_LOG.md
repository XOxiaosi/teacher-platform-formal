...
| 当前任务 | A02-ADAPTER / A03-REC3 / A04-MULTI：固定 DSH 本地适配、可靠重试与多候选兼容迁移 |
| 当前任务状态 | 进行中（A02-ADAPTER/A03-REC3/A04-MULTI 本地工程完整检查已通过，正在分包提交；A02/A03/A04 总体验收仍有下游和真实条件） |
...
| 交付门禁 | 通过（本波本地工程；真实模型、设备和用户验收未验证） |
| 验证概况 | 本轮第二次完整 check 退出 0：后端 306 文件/2725 项，前端 43 文件/268 项，管理端 13 文件/84 项，运维 124 项通过、2 项 Windows 专属跳过；治理 19 项、长度、类型、lint、隔离 PostgreSQL 17 与构建通过。固定 DSH 11 场景、38 项迁移与正式浏览器专项已复验 |
| 下一步 | 完整门禁收口并按 A02/A03/A04 分包提交；立即推进 A05-EVIDENCE 统一反馈依据准入，随后草稿幂等保存/任务接续；真实生成和跨设备 Gate 单列 |
| 长任务目标及结束条件 | 持续推进 V009 当前计划已授权本地工程；每包核验、完整门禁、日志和 commit 后立即安排下游。仅整体完成、用户停止，或剩余任务均需决定/外部条件且无独立可执行工作时结束；真实模型和设备验收单列 |
| 当前可执行任务 | 本波已完成验证并分包提交；随后立即启动 A05-EVIDENCE、A04-DETAIL 与 P6-EXPORT，主 Agent 负责来源详情/分享范围接口、集成、独立审阅和完整检查 |
