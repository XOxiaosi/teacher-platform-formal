...
| manifest 声明目标存在但实际缺失或 T 内容已改 | 重分类后重新生成；来源 A 当前为 M1 114、T 780，包含 V002/V003 网页、T-014 账号隔离、T-015 文字原始事件和 T-016 核心教学适配；962 个目标路径 0 missing，780 个 T 0 mismatch，5 个分片哈希匹配 |
| 包内 `PRODUCT.md` 形成第二事实源 | 从目标移除并记 H；最终只保留根 `PRODUCT.md` |
| 文件长度 Gate 以目录级豁免掩盖遗留 | 改为 26 个路径、行数、SHA-256 精确例外，新增/修改文件仍受 500 行限制；T-014 已拆分原超长管理路由 |
...
- 未执行云端、容器提升、备份恢复演练、150 人容量或成本验证；T-014 只完成本地合成数据的跨教师攻击矩阵。
- 未通过严格 500 行 Gate：当前有 26 个 legacy 精确例外。
- 未退出 legacy-only：当前有 780 个原样 T 项，统一绑定 T-033；114 个根合并、安全、产品、账号隔离、文字原始事件或核心教学适配来源 A 文件已如实记为 M1。
...
- 产品已发布 V003 并完成 T-014/T-015；其余旧前端可构建和测试不代表正式 UI、产品闭环或生产可用。
