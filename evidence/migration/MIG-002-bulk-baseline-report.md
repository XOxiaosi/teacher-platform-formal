...
>
> 历史关闭：同日阶段 0 曾关闭 T-029/T-030；这不是当前根测试通过。2026-09-08 根测试仍有 5 项既有失败；V004 教师旧 UI 已退役，清场记录见 `UI-RESET-retired-files.json` 与 `UI-RESET-validation.md`，当前执行状态只见 `PROJECT_LOG.md`。
>
...
|---|---:|---:|---:|---:|---:|---:|
| A 固定 Git 快照 | 1161 | 88 | 706 | 367 | 0 | 0 |
| B 旧正式目录 | 79 | 33 | 0 | 18 | 28 | 0 |
| N 迁移前基线 | 17 | 0 | 0 | 4 | 0 | 13 |

2026-09-08 V004 清场后：来源 A 1161/1161 唯一覆盖，5 个分片哈希与索引相符，826 个声明目标均存在且当前哈希匹配。旧 UI 退役路径改为 H，不再假报 present；源码外备份与退役前哈希见清场清单。剩余 706 个 T 项仍绑定 T-033，其中 19 项已有内容变更且 contentMatchesSource=false（本轮未改动其业务代码），不能再表述为全部原样或正式验收已通过。
...
- 文件长度检查对新增或修改文件执行 500 行上限；另有 26 个按路径、行数和 SHA-256 精确绑定的 legacy 例外，统一由 T-033 清零。T-014 已拆分原超长管理路由；唯一 Prisma schema 因新增邀请模型重新锁定精确哈希。此结果不等于严格 500 行 Gate 已通过。
- 正式 `api-contracts/domain` 不反向依赖 legacy 包；普通根命令不再暴露会读取 `.env`、迁移数据库或继承环境密钥的旧启动器。

## 5. 迁入时验证证据（历史，不适用于当前 UI）

| 验证 | 结果 | 证据摘要 |
|---|---|---|
| `npm ci` | PASS / 0 | 安装前后 lock SHA-256 均为 `66025d5240b8bde503ea88a370041fa503ee418ee743db5c2668040f7d214067` |
| `npm run check:file-size` | PASS / 0 | 新增/修改文件通过；26 个精确 legacy 例外如实输出 |
...
- T-033：未开始。当次历史快照共有 878 个 legacy-only 文件和 27 个超长文件精确例外；阶段 0 至 T-016 已将其中 98 个文件重分类为 M1，当前剩余 780 个原样 T 仍须随正式模块替换退出。
...
因此阶段 0 当时关闭 T-029/T-030，但历史结论不扩张为“产品完成”“严格 500 行通过”“生产就绪”或“可以上线”。MIG-002 执行时 `PRODUCT.md` 仍为 DRAFT，其后的 V003 状态及 780 个 T/114 个 M1/26 个长度例外亦属于清场前记录；V004 当前数量见第 3 节与清场证据。Windows 11 子进程树、local-safe 非生产身份 fallback 的错误信息卫生、外部服务、云端、容量和供应链 audit 告警仍由后续 Gate 处理。
